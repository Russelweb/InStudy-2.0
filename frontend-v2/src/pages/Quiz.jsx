...
function preprocessMath(text) {
  ...
  return text
    ...
    .replace(
      /\b(sin|cos|tan|cot|sec|csc|ln|log|exp|sqrt|lim|sum|int)\s*\(([^)]+)\)/g,
      (m) => `$${m}$`,
    )
    .replace(/\b([a-zA-Z])'+'?\s*\([^)]+\)/g, (m) => `$${m}$`)
    .replace(
      /\b(alpha|beta|gamma|delta|epsilon|theta|lambda|mu|pi|sigma|omega|phi|psi)\b/g,
      (m) => `$\\${m}$`,
    )
    .replace(
      /([a-zA-Z][a-zA-Z0-9'_^()*/+\-\s]*=[a-zA-Z0-9'_^()*/+\-\s]+)/g,
      (m) => {
...
const QuizStepHeading = ({ eyebrow, title, description }) => (
  <div className="mb-6">
    ...
    <h2 className="text-xl md:text-2xl font-black text-on-surface tracking-tight uppercase italic">
      {title}
    </h2>
...
const QuizReviewItem = ({ icon, label, value }) => (
  <div className="flex items-center gap-3 p-3 rounded-lg bg-surface-container-low border border-outline-variant/5">
    ...
    <div className="min-w-0">
      <p className="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-tighter leading-none mb-1">
        {label}
      </p>
    ...
    >
      <div className="mb-6 text-center">
        ...
        <p className="text-on-surface-variant text-sm mt-2">
          Set up a focused practice quiz without the clutter.
        </p>
      </div>
        ...
        {[
          ...
          return (
            <button
              key={step.label}
              onClick={() =>
                (index === 0 || selectedCourse) && setActiveStep(index)
              }
            ...
            >
              <div className="flex items-center gap-2">
                <span
                  className={`h-8 w-8 rounded-xl flex items-center justify-center text-xs font-black border ${done ? "bg-secondary text-on-secondary border-secondary" : active ? "bg-primary text-on-primary border-primary" : "bg-surface-container-high text-on-surface-variant border-outline-variant/15"}`}
                >
                  {done ? (
                ...
                <span className="min-w-0">
                  <span
                    className={`block text-[10px] font-black uppercase tracking-widest ${active || done ? "text-secondary" : "text-on-surface-variant"}`}
                  >
                    {step.label}
                  </span>
              ...
              <div className="mt-2 h-1 rounded-full bg-surface-container-highest overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${active || done ? "w-full bg-secondary" : "w-0 bg-secondary"}`}
                />
              </div>
            </button>
          );
        ...
        <AnimatePresence mode="wait">
          {activeStep === 0 && (
            ...
            >
              <QuizStepHeading
                ...
                description="Pick the course you want to test yourself on."
              />
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-on-surface-variant mt-1">
                          {selectedCourseData
                            ? `${selectedCourseData.document_count || 0} documents ready`
                      ...
                      {filteredCourses.map((course) => {
                        ...
                        return (
                          <button
                            ...
                            className={`h-14 px-3 rounded-lg border text-left transition-all flex items-center gap-3 min-w-0 ${active ? "bg-secondary/10 border-secondary text-secondary" : "bg-surface-container-low border-outline-variant/10 text-on-surface-variant hover:border-secondary/35 hover:text-on-surface"}`}
                          >
                            <span className="material-symbols-outlined text-lg shrink-0">
                              {active ? "check_circle" : "school"}
                            </span>
                            <span className="min-w-0">
          ...
          {activeStep === 1 && (
            ...
            >
              <QuizStepHeading
                eyebrow="Step 2"
                title="Set Quiz Rules"
                description="Choose the length, difficulty, and question format."
              />
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-5">
                <QuizOptionCard label="Questions">
                  <QuizSegment
                    values={[5, 10, 20]}
                    value={count}
                    onChange={setCount}
                ...
                <QuizOptionCard label="Difficulty">
                  <QuizSegment
                    values={["Easy", "Moderate", "Extreme"]}
                    value={difficulty}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`py-2.5 rounded-xl border text-xs font-black transition-all ${quizType === type.id ? "bg-primary/20 border-primary text-primary" : "border-outline-variant/20 text-on-surface-variant hover:border-primary/40 hover:text-on-surface"}`}
                      >
                        {type.label}
                      </button>
                    ))}
          ...
          {activeStep === 2 && (
            ...
            >
              <QuizStepHeading
                ...
                description="Add an optional focus topic and choose whether to use a timer."
              />
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_0.85fr] gap-4 mt-5">
                <div className="space-y-4">
                  ...
                  <div
                    className={`flex items-start gap-4 p-4 rounded-xl border transition-all ${timedMode ? "bg-primary/5 border-primary/20" : "bg-surface-container border-outline-variant/15"}`}
                  >
                    <span
                      className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${timedMode ? "bg-primary/15 text-primary" : "bg-surface-container-highest text-on-surface-variant"}`}
                    >
                    ...
                    <button
                      ...
                      className="shrink-0 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border border-primary/30 text-primary hover:bg-primary/10 transition-all"
                    >
                ...
                <div className="bg-surface-container-high/60 border border-outline-variant/15 rounded-xl p-4 space-y-2">
                  <QuizReviewItem
                    icon="menu_book"
                    label="Course"
                    value={selectedCourseData?.name || "No course selected"}
                  />
                  <QuizReviewItem
                    icon="quiz"
                    label="Questions"
    ...
    const idleTimer = setTimeout(() => {
      triggerAura(
        ...
        "You've been on this question for a while. Need a subtle hint?",
        {
          ...
          onClick: () =>
            askAuraBackground(
              `Give me a subtle hint for this question without giving away the answer. End by encouraging me to try again. Question: "${currentQ.question}"`,
            ),
        },
        10000,
      );
          ...
          <div className="flex items-center gap-4 sm:gap-8 w-full sm:w-auto justify-between sm:justify-end">
            {timedMode && (
              <div className="text-right">
                <div
                  className={`text-[10px] uppercase tracking-[0.2em] font-bold ${timeLeft <= 60 ? "text-error animate-pulse" : "text-on-surface-variant/60"}`}
                >
                  Time Left
                </div>
                <div
                  className={`text-2xl font-mono tracking-tighter tabular-nums ${timeLeft <= 60 ? "text-error animate-pulse" : "text-primary"}`}
                >
                  {formatTime(timeLeft)}
                </div>
              </div>
              ...
              <div className="w-14 h-14 rounded-full border-2 border-surface-container-highest flex items-center justify-center relative bg-surface-container/30 backdrop-blur-sm">
                ...
                >
                  ...
                  <motion.circle
                    ...
                    animate={{
                      strokeDashoffset:
                        150.8 - 150.8 * ((currentIndex + 1) / questions.length),
                    }}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="space-y-6">
                ...
                <div className="relative">
                  <span className="absolute -left-8 sm:-left-12 top-0 text-4xl sm:text-6xl font-black text-primary/5 select-none leading-none">
                    Q
                  </span>
                            ...
                            <span
                              className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 text-xs font-black transition-colors ${
                                isSelected
                                  ...
                                  : "bg-surface-container-highest text-on-surface-variant group-hover:bg-secondary/20 group-hover:text-secondary"
                              }`}
                            >
                              {ids[i]}
                            </span>
                            <p
                              className={`text-sm sm:text-base leading-snug flex-1 ${isSelected ? "text-on-surface font-bold" : "text-on-surface-variant group-hover:text-on-surface"}`}
                            >
                              {opt}
                            </p>
                            {isSelected && (
    ...
    <div className="flex flex-col lg:flex-row gap-6 lg:gap-10 h-full">
      ...
      <div className="hidden lg:flex lg:w-1/3 lg:flex-col lg:h-full">
        <div className="glass-panel rounded-3xl p-8 flex flex-col items-center text-center border border-outline-variant/10 shadow-2xl relative overflow-hidden group">
          <h3 className="text-[10px] text-on-surface-variant font-black uppercase tracking-[0.4em] mb-12">
            Quiz Summary
          </h3>
          ...
          <div className="relative w-56 h-56 mb-10 flex items-center justify-center mx-auto">
              ...
              <motion.circle
                ...
                animate={{
                  strokeDashoffset:
                    565.5 -
                    565.5 *
                      (Math.min(results?.score_percentage || 0, 100) / 100),
                }}
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              >
                ...
                <span className="text-2xl text-on-surface-variant/40 ml-1">
                  %
                </span>
              </motion.span>
          ...
          <div className="space-y-4 relative z-10 w-full">
            <h4
              className={`font-black text-xl uppercase tracking-tighter ${results?.score_percentage >= 80 ? "text-secondary" : results?.score_percentage >= 50 ? "text-primary" : "text-error"}`}
            >
              {results?.score_percentage >= 80
                ? "Excellent"
                : results?.score_percentage >= 50
        ...
        <div className="flex flex-col gap-3 mt-6">
          <button
            onClick={onSave}
            disabled={isSaving}
            className="w-full py-4 bg-secondary/10 border border-secondary/20 text-secondary rounded-2xl font-black shadow-lg hover:bg-secondary/20 active:scale-[0.98] transition-all text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-2 disabled:opacity-50 group"
          >
            <span className="material-symbols-outlined text-lg">
              {isSaving ? "sync" : "bookmark"}
            </span>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={onRestart}
              className="py-4 bg-surface-container-highest border border-outline-variant/10 text-on-surface rounded-2xl font-black hover:bg-surface-variant active:scale-[0.98] transition-all text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-2"
            >
            ...
            <button
              onClick={onRestart}
              className="py-4 bg-primary text-on-primary rounded-2xl font-black shadow-xl hover:shadow-primary/20 active:scale-[0.98] transition-all text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-2 group"
            >
      ...
      <div className="lg:w-2/3 w-full flex flex-col h-full">
        ...
        <div className="lg:hidden mb-6">
          <div className="glass-panel rounded-2xl p-6 flex flex-col items-center text-center border border-outline-variant/10 shadow-2xl relative overflow-hidden group">
            <h3 className="text-[10px] text-on-surface-variant font-black uppercase tracking-[0.4em] mb-6">
              Quiz Summary
            </h3>
            ...
            <div className="relative w-36 h-36 mb-6 flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                >
                  {Math.round(results?.score_percentage || 0)}
                  <span className="text-lg text-on-surface-variant/40 ml-1">
                    %
                  </span>
                </motion.span>
                <span className="text-[10px] text-on-surface-variant/60 font-bold uppercase tracking-widest">
            ...
            <div className="space-y-3 relative z-10 w-full">
              <h4
                className={`font-black text-lg uppercase tracking-tighter ${results?.score_percentage >= 80 ? "text-secondary" : results?.score_percentage >= 50 ? "text-primary" : "text-error"}`}
              >
                {results?.score_percentage >= 80
                  ? "Excellent"
                  : results?.score_percentage >= 50
            ...
            {results?.question_results?.map((q, idx) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${q.is_correct ? "bg-secondary/10 text-secondary" : "bg-error/10 text-error"}`}
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                  <div
                    className={`p-4 rounded-2xl border ${q.is_correct ? "bg-secondary/5 border-secondary/10" : "bg-error/5 border-error/10"}`}
                  >
                    ...
                    <p
                      className={`text-sm font-bold ${q.is_correct ? "text-secondary" : "text-error"}`}
                    >
                  ...
                  {!q.is_correct && (
                    <div className="p-4 rounded-2xl bg-secondary/5 border border-secondary/10">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-[9px] font-black uppercase tracking-widest opacity-40 text-on-surface-variant">
                          Correct Answer
                        </span>
                      </div>
                ...
                <div className="mt-6 p-5 rounded-2xl bg-surface-container-low border border-outline-variant/5">
                  ...
                  <div className="text-sm text-on-surface-variant leading-relaxed font-medium markdown-content">
                    <ReactMarkdown
                      ...
                      components={{
                        h1: ({ children }) => (
                          <h1 className="text-base font-black text-secondary mt-4 mb-2 pb-1 border-b border-secondary/20">
                            {children}
                          </h1>
                        ),
                        h2: ({ children }) => (
                          <h2 className="text-sm font-bold text-on-surface mt-3 mb-2">
                            {children}
                          </h2>
                        ),
                        h3: ({ children }) => (
                          <h3 className="text-xs font-bold text-secondary/80 mt-2 mb-1">
                            {children}
                          </h3>
                        ),
                        p: ({ children }) => (
                          <p className="my-2 leading-relaxed text-on-surface-variant/90">
                            {children}
                          </p>
                        ),
                        ul: ({ children }) => (
                          <ul className="my-2 pl-4 space-y-1 list-disc text-on-surface-variant">
                            {children}
                          </ul>
                        ),
                        ol: ({ children }) => (
                          <ol className="my-2 pl-4 space-y-1 list-decimal text-on-surface-variant">
                            {children}
                          </ol>
                        ),
                        li: ({ children }) => (
                        ...
                        code: ({ inline, children }) =>
                          inline ? (
                            <code className="bg-secondary/10 px-1 py-0.5 rounded text-[12px] text-secondary font-mono font-medium">
                              {children}
                            </code>
                          ) : (
                            <div className="relative group my-3">
                              <pre className="bg-surface-container-highest/50 rounded-xl p-3 overflow-x-auto border border-outline-variant/10 shadow-inner">
                                <code className="text-xs text-secondary font-mono leading-normal">
                                  {children}
                                </code>
                              </pre>
                            </div>
                        ...
                        blockquote: ({ children }) => (
                          <blockquote className="border-l-4 border-secondary/30 bg-secondary/5 pl-3 py-1.5 my-3 text-on-surface-variant italic rounded-r-lg">
                            {children}
                          </blockquote>
                        ),
                        table: ({ children }) => (
                        ...
                        th: ({ children }) => (
                          <th className="px-3 py-2 text-left font-black text-secondary border-b border-outline-variant/20 uppercase tracking-wider text-[10px]">
                            {children}
                          </th>
                        ),
                        td: ({ children }) => (
                          <td className="px-3 py-2 border-b border-outline-variant/10 text-on-surface-variant leading-relaxed">
                            {children}
                          </td>
                        ),
                        a: ({ href, children }) => (
                          <a
                            ...
                            className="text-secondary underline decoration-secondary/30 underline-offset-2 hover:text-secondary-fixed transition-colors font-medium"
                          >
                            {children}
                          </a>
                        ),
...
const Quiz = () => {
  ...
  useAuraHelp(
    "Choose your course, set difficulty and question count, then click Generate Quiz. Your results update your Mastery score.",
  );
  ...
  useEffect(() => {
    ...
    try {
      legacyAssetRaw = localStorage.getItem("load_asset_quiz");
    } catch (_) {}

    const assetToLoad =
      routerAsset || (legacyAssetRaw ? JSON.parse(legacyAssetRaw) : null);
    const effectiveMode = routerAsset
      ? loadMode
      : assetToLoad?.data?.results
        ? "results"
        : "retake";
    ...
    if (assetToLoad) {
      try {
        console.log(
          `Loading saved quiz: "${assetToLoad.title}" in ${effectiveMode} mode`,
        );
  ...
  useEffect(() => {
    ...
    if (selectedCourse)
      localStorage.setItem("quiz_selected_course", selectedCourse);
    if (evaluationResults)
      localStorage.setItem("quiz_results", JSON.stringify(evaluationResults));
  ...
  const handleStartQuiz = async (
    ...
    try {
      ...
      setCurrentQuestions(
        response.data.questions || response.data.quiz?.questions || [],
      );
      setPhase("assessment");
    } catch (error) {
      triggerAura(
        "concerned",
        "Couldn't generate the quiz. Make sure this course has documents uploaded.",
        {
          label: "Go to Knowledge Base",
          onClick: () => navigate("/knowledge"),
        },
