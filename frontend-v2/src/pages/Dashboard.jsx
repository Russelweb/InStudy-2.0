  ...
  return (
    <div className="bg-surface-container-high border border-outline-variant/20 rounded-xl px-4 py-3 shadow-2xl text-xs">
      ...
      <p className="text-secondary">
        <span className="font-black">{payload[0]?.value?.toFixed(2)}h</span>{" "}
        study time
      </p>
      {payload[0]?.payload?.questions > 0 && (
        <p className="text-primary mt-0.5">
          {payload[0].payload.questions} questions asked
        </p>
      )}
    </div>
  );
      ...
      <div className="flex gap-2">
        {days.map((day, i) => (
          <div key={i} className="flex-1 flex flex-col items-center gap-2">
            ...
            {/* Day label */}
            <span
              className={`text-[10px] font-bold uppercase tracking-wider ${day.isToday ? "text-secondary" : "text-on-surface-variant/60"}`}
            >
              {day.label}
            </span>
          </div>
      ...
      <div className="flex items-center gap-2 mt-3">
        ...
        {[0.04, 0.2, 0.4, 0.65, 0.9].map((op, i) => (
          <div
            ...
            style={{ background: `rgba(105,246,184,${op})` }}
          ></div>
        ))}
        <span className="text-[9px] text-on-surface-variant uppercase tracking-widest">
          More
      ...
      {hasData ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Tooltip
                ...
                cursor={{ stroke: "rgba(105,246,184,0.2)", strokeWidth: 2 }}
              />
              <Area
                type="monotone"
                dataKey="hours"
        ...
        <div className="h-64 flex flex-col items-center justify-center gap-4 border border-dashed border-outline-variant/20 rounded-xl">
          ...
          <p className="text-on-surface-variant/60 text-xs">
            Start asking questions in the AI Tutor to generate activity.
          </p>
        </div>
      )}
    </div>
...
const OnboardingChecklist = ({ stats, loading }) => {
  ...
  const hasFlashcards =
    !loading &&
    Object.values(stats.daily_activity || {}).some(
      (day) => (day.flashcards || 0) > 0,
    );
  const hasActivity =
    !loading && (stats.quizzes_taken > 0 || stats.study_hours > 0);
        ...
        {steps.map((step, i) => (
          ...
          >
            ...
            <div className="min-w-0">
              <p
                className={`text-sm font-bold leading-tight ${step.done ? "text-secondary line-through opacity-60" : "text-on-surface"}`}
              >
                {step.title}
              </p>
  ...
  <Link to={path} className="block h-full">
    ...
    >
      <div className="flex justify-between items-start mb-3 sm:mb-4">
        <span
          className={`material-symbols-outlined text-${accentColor} text-2xl sm:text-3xl`}
        >
          {icon}
        </span>
        <span
          className={`text-[9px] sm:text-[10px] font-bold text-${accentColor} tracking-widest`}
        >
          {change}
        </span>
      </div>
      <h3 className="text-on-surface-variant text-[10px] sm:text-xs font-bold uppercase tracking-widest mb-1">
        {title}
      </h3>
...
const Dashboard = () => {
  ...
  useAuraHelp(
    "This is your Dashboard — track study hours, mastery progress, and recent activity. Start by creating a course in Knowledge Base.",
  );
    ...
    const fetchTodayXp = async () => {
      try {
        ...
        setTodayXp(
          results
            .filter(
              (r) => r.status === "fulfilled" && r.value.total_xp_today > 0,
            )
            .map((r) => r.value),
        );
      } catch {
  ...
  useEffect(() => {
    ...
    masteryService.v2
      .getStale(activeCourse.id, 3)
      .then((res) => {
        const stale = res.data?.stale_subtopics || [];
        if (stale.length > 0) {
          sessionStorage.setItem("aura_nudge_shown", "true");
          const top = stale[0];
          const decay = top.decay_amount
            ? ` (–${top.decay_amount.toFixed(0)}%)`
            : "";
          triggerAura(
            "nudge",
            `"${top.concept_name}" is decaying${decay}. A quick review will lock it in.`,
            {
              ...
              onClick: () => navigate(`/flashcards?id=${activeCourse.id}`),
            },
            9000,
          );
        } else {
              ...
              if (legacyStale.length > 0) {
                ...
                triggerAura(
                  ...
                  `Your mastery of "${concept}" has started to decay. A quick review will lock it in.`,
                  {
                    label: "Review Flashcards",
                    onClick: () =>
                      navigate(`/flashcards?id=${activeCourse.id}`),
                  },
                  9000,
                );
              }
  ...
  const allRows = useMemo(
    () => buildDailyRows(stats.daily_activity),
  ...
  const topCourse =
    stats.courses.length > 0
      ? stats.courses.reduce((a, b) => (a.mastery > b.mastery ? a : b))
      : null;
  const auraInsight = topCourse
    ? `You are ${topCourse.mastery}% through mastering ${topCourse.name}.`
    : "Upload your first document to begin.";
  const topConcepts = [...(stats.courses || [])]
    .sort((a, b) => b.mastery - a.mastery)
    .slice(0, 3);
  const recentQueries = [...(stats.recent_questions || [])]
    .reverse()
    .slice(0, 3);
  ...
  return (
    <div className="p-4 md:p-8 pb-12">
      ...
      >
        <div>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-black tracking-tighter text-on-surface mb-1">
            Welcome back, {displayName}!
          </h1>
          <p className="text-on-surface-variant text-xs sm:text-sm tracking-wide">
            What course do you plan on overhauling today?
          </p>
        </div>
        <div className="glass px-6 py-3 rounded-full flex items-center gap-3 border-secondary/20 group hover:scale-105 transition-transform duration-500">
          ...
          <span className="text-secondary text-sm font-bold tracking-tight">
            🔥 Aura Insight:{" "}
            <span className="text-on-surface font-normal">{auraInsight}</span>
          </span>
        </div>
      </motion.div>
          ...
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="min-w-0">
              <span className="text-[10px] font-black uppercase tracking-widest text-secondary/70">
                Last Interacted Course
              </span>
              <h3 className="text-lg font-black text-on-surface truncate mt-0.5">
                {lastActiveCourse.name}
              </h3>
              <p className="text-xs text-on-surface-variant/80 mt-1">
                {lastActiveCourse.document_count} Documents ·{" "}
                {lastActiveCourse.mastery}% Mastered
              </p>
            </div>
          </div>
          <Link
            ...
            onClick={() =>
              localStorage.setItem("activeCourse", lastActiveCourse.id)
            }
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {todayXp.map((item, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between p-3 bg-surface-container-low/60 rounded-xl border border-secondary/10"
                >
                  <div className="min-w-0">
                    ...
                    {item.mastery_gained_today > 0 && (
                      <p className="text-[9px] text-on-surface-variant">
                        +{item.mastery_gained_today.toFixed(1)}% mastery
                      </p>
                    )}
                  </div>
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <div className="glass p-8 rounded-2xl">
              ...
              {stats.courses.length > coursesPerPage && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-widest">
                    Page {coursesPage} of{" "}
                    {Math.ceil(stats.courses.length / coursesPerPage)}
                  </span>
                  <button
                    onClick={() =>
                      setCoursesPage((p) =>
            ...
            <div className="space-y-4">
              {loading ? (
                <div className="text-center py-8 text-on-surface-variant text-sm animate-pulse">
                  Loading Courses...
                </div>
              ) : stats.courses.length === 0 ? (
                <div className="text-center py-8 border border-dashed border-outline-variant/20 rounded-xl">
                  <span className="material-symbols-outlined text-3xl text-on-surface-variant mb-2 block opacity-40">
                    inventory_2
                  </span>
                  <p className="text-sm text-on-surface-variant">
                    No courses yet.
                  ...
                  <Link
                    to="/knowledge"
                    className="mt-3 inline-block text-xs font-bold text-secondary uppercase tracking-widest hover:underline"
                  >
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-on-surface-variant uppercase tracking-widest">
                            {course.document_count} Documents Linked
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
            ...
            <div className="space-y-6">
              {topConcepts.length > 0 ? (
                topConcepts.map((course, i) => (
                  ...
                  >
                    <div className="flex justify-between text-sm mb-2">
                      <span className="font-bold truncate pr-2 group-hover:text-primary transition-colors">
                        {course.name}
                      </span>
                ...
                <p className="text-xs text-on-surface-variant text-center py-4">
                  Take quizzes to see mastery scores.
                </p>
              )}
            </div>
            <Link to="/knowledge">
              ...
              {recentQueries.length > 0 ? (
                recentQueries.map((q, i) => (
                  <Link
                    ...
                    onClick={() =>
                      q.course && localStorage.setItem("activeCourse", q.course)
                    }
                    className="block"
                  >
                    <div
                      className={`relative p-3 rounded-lg bg-surface-container-highest/50
    hover:bg-surface-container-highest transition-all cursor-pointer
    after:absolute after:left-0 after:top-1/2 after:-translate-y-1/2
    after:h-10 after:w-[2px]
    ${i % 2 === 0 ? "after:bg-primary" : "after:bg-secondary"}
    after:rounded-full`}
                    >
                ...
                <p className="text-xs text-on-surface-variant text-center py-4">
                  Ask the AI Tutor questions to see them here.
                </p>
              )}
            </div>
          </div>
          ...
          <div className="bg-[#551a8b] p-6 rounded-2xl relative overflow-hidden group cursor-pointer shadow-2xl">
            ...
            <h3 className="text-on-primary-fixed text-xl font-black mb-2 text-white">
              Upgrade to Ultra
            </h3>
            <p className="text-on-primary-fixed/80 text-xs mb-4 text-white">
              Unlock the neural-direct interface and unlimited concept mapping.
            </p>
            <button className="bg-on-primary-fixed text-white px-4 py-2 rounded text-[10px] font-black uppercase tracking-widest">
              Go Ultra
            </button>
          </div>
        </div>
