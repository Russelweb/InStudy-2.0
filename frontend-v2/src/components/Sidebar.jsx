...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { icon: 'quiz', label: 'Quiz Mode', path: '/quiz' },
    { icon: 'smart_toy', label: 'AI Tutor', path: '/ai-tutor' },
    { icon: 'settings', label: 'Settings', path: '/settings' },
  ];

  return (
    <aside className="fixed left-0 top-0 h-full w-64 bg-[#0a0f0b] flex flex-col py-8 px-4 z-50 shadow-[0px_20px_40px_rgba(189,157,255,0.05)]">
      <div className="mb-12 px-4">
        <h1 className="text-2xl font-black text-[#bd9dff] tracking-tighter">InStudy 2.0</h1>
        <p className="text-[10px] uppercase tracking-widest text-on-surface-variant/60 mt-1">Cyber-Elite Scholar</p>
      </div>
      ...
      <div className="mt-auto px-4 flex items-center justify-between gap-3 border-t border-outline-variant/10 pt-6">
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:block">
            ...
            <p className="text-[10px] text-on-surface-variant">Level 42 Architect</p>
          </div>
        </div>
        <button 
